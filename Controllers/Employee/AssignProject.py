...
def assign_project_employee(project):
    ...
    try:
        ...
        cursor_object.execute(f"UPDATE employees_information SET assigned = 1 WHERE id = '{project['employee_id']}'")
        cursor_object.execute(f"UPDATE employees_information SET project_assigned = '{project['project_id']}' WHERE id = '{project['employee_id']}'")
        connection.commit()
