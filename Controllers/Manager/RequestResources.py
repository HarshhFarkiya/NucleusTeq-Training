...
def request_resource(request):
    ...
    try:
        ...
        #Check wether that all exists or not
        cursor_object.execute(f"SELECT * FROM managers_information WHERE id='{id}'")
        check = cursor_object.fetchall()
        if len(check)<=0 :
            return JSONResponse(content=f"{{'message': 'Invalid manager id'}}",status_code=404)
        ...
        check=cursor_object.fetchall()
        if len(check)<=0:
            return JSONResponse(content=f"{{'message': 'Invalid resource id'}}",status_code=404)
