...
def employee_auth(user):
    required_parameters = ['employee_id','token']
    ...
    try:
        ...
        cursor_object.execute(f"SELECT token,user_role FROM user_management WHERE id='{user['employee_id']}'")
        db_token = cursor_object.fetchone()
        if db_token is None:
            return JSONResponse(content={"message": "User not found"}, status_code=404)  
        if db_token[0] != user['token'] or db_token[1]!='employee':
